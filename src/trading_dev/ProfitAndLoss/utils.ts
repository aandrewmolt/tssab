...
export async function loadBoughtTokens(path_To_bought_tokens: string) {
    try {
        ...
        const data = await fsPromises.readFile(path_To_bought_tokens, 'utf8');
...
export async function readBoughtTokens(tokenAddress:string, path_To_bought_tokens:string) {
    try {
        const data = await fsPromises.readFile(path_To_bought_tokens, { encoding: 'utf8' });
        const jsonData = JSON.parse(data);
        if(!(tokenAddress in jsonData)){
            return null;
        }
        return jsonData[tokenAddress];
    } catch (err) {
...
export async function writeBoughtTokens(tokenAddress:string, tokenObj:object, path_To_bought_tokens:string) {
    try {
        ...
        await fsPromises.writeFile(path_To_bought_tokens, JSON.stringify(jsonData, null, 2));
    } catch (err) {
        logger.error('Error writing bought tokens file:', err);
    }
}
/**
...
export async function deleteBoughtTokens(tokenAddress:string, path_To_bought_tokens:string) {
    try {
        ...
        logger.error('Error deleting bought tokens file:', err);
    }
}
...
export async function writeLineToLogFile(logMessage:string) {
    await fsPromises.appendFile(log_path, `${logMessage}\n`, 'utf8');
}
  //"tokenA": {
