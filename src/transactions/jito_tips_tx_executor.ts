...
export async function jito_executeAndConfirm(
  ...
  try {
    ...
    if (success_res.length > 0) {
      ...
      const jitoResult = await jito_confirm(jitoTxSignature, lastestBlockhash);
      if (jitoResult.confirmed) {
        return jitoResult;
      }
    }
    
    // If Jito fails, fall back to regular transaction
    ...
    console.log("Error during transaction execution: ", e);
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (retryCount === maxRetries) {
        console.log("Max retries reached for transaction confirmation");
        return { confirmed: false, signature };
      }
      console.log(`Retry attempt ${retryCount} for confirmation`);
      // Exponential backoff
