 ...
 */
async function swapOnlyAmm(input: any): Promise<SwapResult> {
    ...
    instructions: [
      ...[
        ...
        ComputeBudgetProgram.setComputeUnitPrice({
          microLamports: 20_000_000 // About 0.02 SOL priority fee
        }),
      ],
      ...(input.side === "buy"
        ? [
...
export async function swap(
  ...
  if (side === "buy") {
    ...
    if (usage === "volume") {
      const result = await swapOnlyAmm(input) as SwapResult;
      return ''; // Return empty string for volume usage
    }
